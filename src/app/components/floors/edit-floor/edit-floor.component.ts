...
export class EditFloorComponent implements OnInit {
  ...
  addDimension() {
    const dimensionFormGroup = new FormGroup( {
    ...
    this.dimension.push(dimensionFormGroup)
  }
  ...
  submitForm() {
    this.floorService.UpdateFloor(this.floorForm.value.id, this.floorForm.getRawValue).subscribe((res) => {
